...
#
# def model_train(num_epochs, model, train_loader):
#     model.train()
#     for epoch in range(num_epochs):
#         # running_loss = 0.0
...
#             # running_loss += loss
#             pred = output.argmax(dim=1)
#             batch_correct = torch.eq(pred, b_y).sum().float().item()
#             accuracy = batch_correct * 100 / b_y.shape[0]
#
#             loss.backward()
#             optimizer.step()
#             # print(f'Train: Epoch [{epoch + 1}/{num_epochs}], '
#             #       f'Step [{i + 1}/{math.ceil(len(train_loader.dataset) / args.batch_size)}], '
#             #       f'Loss: {loss.item():.4f}, Accuracy: {accuracy:.3f}')
#
#         for param_group in optimizer.param_groups:
#             LR = param_group['lr']
#         print(f'Train: Epoch [{epoch + 1}/{num_epochs}, LR: {LR}]')
#         val_acc = model_test(model, test_loader)
#
#         # writer.add_scalar("Loss/train", running_loss / args.batch_size, epoch + 1)
#         # writer.add_scalar("Accuracy/train", val_acc, epoch + 1)
#
#
