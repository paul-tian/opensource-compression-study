...
fashion_augment_train_transform = transforms.Compose([
...
fashion_origin_train_transform = transforms.Compose([
    transforms.Resize(33),
    transforms.ToTensor(),
    transforms.Normalize((0.2860,), (0.3530,))
])

augment_train_dataset = datasets.FashionMNIST('~/DATASET/PyTorch-Dataset/', train=True, download=True,
                                              transform=fashion_augment_train_transform)
origin_train_dataset = datasets.FashionMNIST('~/DATASET/PyTorch-Dataset/', train=True, download=True,
                                             transform=fashion_origin_train_transform)
...
test_dataset = datasets.FashionMNIST('~/DATASET/PyTorch-Dataset/', train=False, transform=fashion_test_transform)
...
if __name__ == '__main__':
    ...
    model = ResNet18(in_channels=1, mask=True)
    model.load_state_dict(torch.load(args.full))
    model.to(device)
    ...
    torch.save(model.state_dict(), args.prune)
